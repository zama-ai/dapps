...
describe("FHEwordle", function () {
  ...
  it("should setup and initialize game correctly", async function () {
    ...
    const validWordsList = VALID_WORDS.map((word) => [0, wordToNumber(word)]);
  ...
  it("should submit word letters correctly", async function () {
    ...
    const inputl0 = this.instances.createEncryptedInput(this.FHEWordleAddress, this.signers.bob.address);
    const input = inputl0.add8(l0).add8(l1).add8(l2).add8(l3).add8(l4);
    const encryptedAmount = await input.encrypt();

    const tx1 = await this.FHEWordleGame.submitWord1(
    ...
    // Verify game state
    // expect(await this.FHEWordleGame.wordSubmitted()).to.be.true;
    // expect(await this.FHEWordleGame.gameStarted()).to.be.true;
  });
  ...
  //   const factoryContract: FHEWordleFactory = await FHEWordleFactoryFactory
